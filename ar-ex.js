...
function initPermissionScreen() {
  ...
  const items = [{ id: "markCamera", img: "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/camera.png", txt: lang[currentLang]["EXP_INFO_CAMERA_TITLE"] }];
  if (isIOS) {
    items.push({ id: "markMotion", img: "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/motion.png", txt: lang[currentLang]["EXP_INFO_MOTION_TITLE"] });
  }
...
function detectAndroid() {
  // 1) Modern: userAgentData (Chrome 계열)
  const uaData = navigator.userAgentData;
  if (uaData?.platform) {
    return uaData.platform.toLowerCase() === "android";
  }

  // 2) Fallback: userAgent
...
async function requestCamera() {
  ...
  try {
    ...
    isRequesting = false;
    await updatePermissionUI();
    saveUserLog("TICKET - 카메라권한 버튼 클릭");
...
function hideBottomNotice() {
  ...
  const refreshDiv = document.getElementById("refreshBtnDiv");
  if (refreshDiv) refreshDiv.classList.remove("hidden");
}
...
async function loadEvent(isReadyEnd) {
  ...
  var changeNumberEl1 = document.getElementById("arModalInfoMaxTokenContent");
  changeNumberEl1.textContent = maxMealCnt;
  var changeNumberEl2 = document.getElementById("arModalInfoDayTokenContent");
  changeNumberEl2.textContent = WANT_TOKEN_COUNT;
  var changeNumberEl3 = document.getElementById("arModalInfoRewardContent");
  changeNumberEl3.textContent = maxMealCnt;
}
...
async function onCameraReady() {
  ...
  if (overlay && localStorage.getItem(`catchYn${cpncd}`) !== "Y") {
...
function setMealSumList(list, isAni) {
  ...
  if (mealSumList.length > 0) {
    ...
    if (rewardUtilCountDiv) {
      if (rewardUtilCountDiv) {
        const prevUtil = Number(rewardUtilCountDiv.textContent);

        rewardUtilCountDiv.textContent = rewardUtilCount;
        if (Number.isFinite(prevUtil) && Number.isFinite(rewardUtilCount) && rewardUtilCount < prevUtil) {
          requestAnimationFrame(() => animateUtilChange(rewardUtilCountDiv));
        }
      }
    }
...
console.log("a-scene count:", document.querySelectorAll("a-scene").length);
...
AFRAME.registerComponent("nx-nearest-guide", {
  tick: function () {
    ...
    const camThree = sceneEl.camera;
    const camObj = camThree; // THREE.Camera (Object3D)
    camObj.updateMatrixWorld(true);

    const arrowEl = document.getElementById("nxArrow");
    const distEl = document.getElementById("nxDistance");
    const behindEl = document.getElementById("nxBehind");
    ...
    nearestObj.getWorldPosition(nearestWorld);

    const local = nearestWorld.clone();
    camObj.worldToLocal(local);

    distEl.textContent = nxFormatDistance(nearestDist);

    const behind = local.z > 0;
    if (behindEl) behindEl.style.display = behind ? "inline" : "none";

    const dx = local.x; // 오른쪽 + / 왼쪽 -
    const dz = -local.z; // 정면을 +로(three 카메라 forward가 -z라서 뒤집음)

    let yawDeg = (Math.atan2(dx, dz) * 180) / Math.PI;

    if (NX_GUIDE_QUANTIZE_4DIR) {
      yawDeg = Math.round(yawDeg / 90) * 90;
    }
    ...
    // 뒤쪽이면 살짝 강조(선택)
    if (behind) {
      arrowEl.style.filter = "drop-shadow(0 2px 14px rgba(255,80,80,0.65))";
    } else {
      arrowEl.style.filter = "drop-shadow(0 2px 10px rgba(0,0,0,0.45))";
    }
  },
});
...
AFRAME.registerComponent("billboard", {
  tick: function () {
    if (!this.el.object3D || !this.el.object3D.position) return;
    ...
    object3D.getWorldPosition(objectWorldPos);

    // y값은 토큰 높이로 맞춤
    cameraWorldPos.y = objectWorldPos.y;
...
AFRAME.registerComponent("proximity-visibility", {
  schema: {
    maxDistance: { type: "number", default: 8 }, // 보여줄 거리 (m) - 넉넉히
    hysteresis: { type: "number", default: 1.5 }, // 완충 구간 (m)
    tickMs: { type: "number", default: 120 }, // 너무 자주 계산하지 않게
  },
  init: function () {
    this._last = 0;
    this._shown = true;
  ...
  tick: function (t) {
    if (t - this._last < this.data.tickMs) return;
    this._last = t;
    ...
    // ✅ 히스테리시스: 경계 근처에서 깜빡이지 않게
    if (this._shown) {
      // 숨길 땐 더 멀어져야 숨김
      if (dist > this.data.maxDistance + this.data.hysteresis) {
        this._shown = false;
        this.el.object3D.visible = false;
      }
    } else {
      // 다시 보일 땐 충분히 가까워져야 보임
...
function renderTokens_old() {
  ...
  mealList.forEach((place, index) => {
    ...
    if (place.model.endsWith(".glb") || place.model.endsWith(".gltf")) {
      ...
      image.setAttribute("src", `${selectedTokenImg}`);
      ...
      var dataTestmealSumList = [
        {
          ...
          MH_TYPE: "2",
          MH_IMG: "/images/meal/00000000_00000000_0000000000000.png",
          MH_SDATE: "20260101",
          MH_EDATE: "20261231",
          MH_NAME: "미르 적립(기본)",
        ...
        },
        {
          ...
          MH_TYPE: "2",
          MH_IMG: "/images/meal/00000000_00000000_0000000000000_2.png",
          MH_SDATE: "20260101",
          MH_EDATE: "20261231",
          MH_NAME: "미르 적립(추가)",
...
async function startPortalLottie() {
  ...
  portalAnim = lottie.loadAnimation({
    ...
    loop: false,
    autoplay: true,
    path: "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/lottie/portal_ntokozo.json",
  });
}
...
async function spawn2DAroundCamera(mealList, radius) {
  ...
  for (let i = 0; i < mealList.length; i++) {
    ...
    const img = place?.MH_IMG || "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/KR_bibimbap.png";
      ...
      try {
        if (USE_CATCH_FETCH) {
          ...
          if (data.proc_code === "0000") {
            markCaught(seq);
            collectedCnt += 1;
            localStorage.setItem(`catchYn${cpncd}`, "Y");
...
async function getMealsForSpawn() {
  const imgs = ["https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/coin_buddha_statue.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/coin_gold_bar.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/coin_money_1.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/jewel_diamond.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/people_girl_1.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/people_girl_2.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/people_girl_3.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/people_girl_4.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/people_girl_5.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/people_man_1.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/people_man_2.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/people_man_3.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/cosmetics_cream.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/cosmetics_lipstick.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/item_flower.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/car_1.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/item_skeleton.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/drink_americano.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/drink_beer.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/drink_chamisul.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/drink_champagne.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/drink_cocktail_1.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/drink_cocktail_2.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/drink_whiskey.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/drink_wine.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/food_hotdog.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/food_kimbap.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/food_macaron.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/JP_sushi.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/food_watermelon.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/ID_barong_mask.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/ID_garuda.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/IN_elephant.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/IN_lionstatue.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/TH_buddha_statue.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/TH_dragon_sculpture.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/US_route66.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/US_statue_of_liberty.png", "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/token/VN_conicalhat.png"];
...
let BUCKETS = [
  { minW: 0.0, maxW: 0.25, min: 2, max: 4 },
  { minW: 0.25, maxW: 0.5, min: 4, max: 7 },
  { minW: 0.5, maxW: 0.75, min: 7, max: 11 },
  { minW: 0.75, maxW: 1.0, min: 11, max: 16 },
];
...
function connectWebSocket() {
  ...
  webSocket.onmessage = (event) => {
    try {
      ...
      var remainCnt = Number(webResponse.data?.remainCnt ?? currentRewardCnt);
...
function updateTokenUI(remainCnt, totalCnt, div) {
  ...
  if (totalMiniEl) totalMiniEl.innerHTML = totalCnt;
  if (remainMiniEl) remainMiniEl.innerHTML = remainCnt ?? currentRewardCnt;
...
function animateUtilChange(el) {
  if (!el) return;
  el.classList.remove("pop-util");
  void el.offsetWidth;
  el.classList.add("pop-util");
}
...
function setAuthModalText(type) {
  ...
  el_authMidImg.src = "../AR-NX/PLAY/img/auth/and_camera_1.png";
  ...
  if (isIOS2()) {
    el_authMidImg.src = "../AR-NX/PLAY/img/auth/ios_camera_1.png";
...
async function preloadLottie() {
  giftJson = await fetch("https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/lottie/gift_box.json").then(r => r.json());
  fireworksJson = await fetch("https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/lottie/fireworks.json").then(r => r.json());
}

function refreshBtnClick() {
  saveUserLog("TICKET - 새로고침");
  location.reload();
}