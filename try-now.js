...
function initPermissionScreen() {
  ...
  items.push({ id: "markCamera", img: "https://cdn.jsdelivr.net/gh/wowinfotechkr/inspire-view@v1.3.2/img/camera.png", txt: lang[currentLang]["EXP_INFO_CAMERA_TITLE"] });
...
function resetPermissionBottomUI() {
  ...
  const nextBtn = document.getElementById("btn-next");
  const refreshBtn = document.getElementById("refreshNotice");

  if (notice) notice.classList.add("hidden"); // 안내창 숨김
  if (refreshBtn) refreshBtn.classList.add("hidden"); // 안내창 숨김
...
async function initPermissionFlow() {
  ...
  if (camOK && (needsMotion() ? motionGranted : true)) {
    ...
    loadEvent(true);
	  ...
	  if(checkParmasFacebook){
	      fMaxProgress = 100;
	      showScreen("tryGameScreen2");
	      showOverlay("permissionScreen");
	  }else{
	  showScreen("tryGameScreen1");
	    initTryGameScreen1();
		startTryGameTokenIntro();
  ...
  try {
    ...
    if (typeof DeviceOrientationEvent !== "undefined" && typeof DeviceOrientationEvent.requestPermission === "function") {
      ...
      if (res === "granted") {
        ...
        //loadEvent(true);
        //document.getElementById("btn-next").innerText = `${lang_EXP_BTN}`;
        saveUserLog("TRY - 모션권한 허용");
        
        setTimeout(function() {
          if (!(camOK || camDenied || isRequesting)) requestCamera(true)
        }, 1000);
      } else {
        motionGranted = false;
        motionDenied = true;
        ...
        showBottomNotice(`${lang_RETRY_REQ}`);
        saveUserLog("TRY - 모션권한 거부");
      }
    } else {
      saveUserLog("TRY - 모션권한 버튼 클릭 - 스킵");
...
async function spawn2DAroundCamera(mealList, radius) {
  ...
  const distance = 9;
  const spawnPos = camWorldPos.clone().add(forward.multiplyScalar(distance));

  //const points = generatePoints(mealList.length, radius, TOKEN_MIN_SPACING);
  const points = [];
  //points.push({x: 0, y: 2.9, z: -9});
  points.push({ x: spawnPos.x, y: spawnPos.y, z: spawnPos.z });
